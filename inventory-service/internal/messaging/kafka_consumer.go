...
type KafkaConsumer interface {
	ConsumeOrderEvents(ctx context.Context, inventoryService service.InventoryService) error
	Close() error
}
...
func (c *kafkaConsumer) ConsumeOrderEvents(ctx context.Context, inventoryService service.InventoryService) error {
	for {
		...
		if err := inventoryService.HandleOrderEvent(ctx, &orderEvent); err != nil {
